...
def floor_div(args):
    ...
    if args is nil:
        ...
        while pair is not nil:
            result = result // calc_eval(pair.first)
            pair = pair.rest
        return result
...
def eval_and(expressions):
    ...
    while expression is not nil:
        first = calc_eval(expression.first)
        if first is scheme_f:
            return scheme_f
        elif first is not scheme_t:
            result= first
...
def eval_define(expressions):
    ...
    result = calc_eval(rest)

    if result is nil:
        bindings[first] = rest.first
    else:
        bindings[first] = result
