...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        symbol = signature.first    #上面例子中的f
        formals = signature.rest    #例子中的(x y)
        body = expressions.rest     #相当于 ((+ x y))
        lambda_procedure = do_lambda_form(Pair(formals, body), env) #在对应的环境中构造lambda表达式,相当于例子中的((x y) (+ x y))
        env.define(symbol, lambda_procedure)    #函数名绑定到对应的lambda中
        return symbol
    else:
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_scheme_true(test):
            expression = clause.rest
            if expression is nil:
                return test
            else:
                return eval_all(expression, env)
        expressions = expressions.rest
    return None
...
def make_let_frame(bindings, env):
    ...
    names = vals = nil
    while bindings is not nil:      #构造临时变量和对应的值
        binding = bindings.first
        validate_form(binding, 2, 2)    #确保每组表达式只能有两个部分:name和对应的val

        name = binding.first        
        val = scheme_eval(binding.rest.first, env)  #计算对应name的val
